/************************************************************************************************
* @file    pg.h
* @date    2018/8/22 19:29:32
* @brief   ParameterGroup, ??∞ÈõÜ?àÁÆ°Áê?
* @author  qyx
...
#define PGR_PGN_MASK            0x0fff
#define PGR_PGN_VERSION_MASK    0xf000
#define PGR_SIZE_MASK           0x0fff
#define PGR_SIZE_SYSTEM_FLAG    0x0000 // documentary

// function that resets a single parameter group instance
typedef void (pgResetFunc)(void * /* base */, int /* size */);
typedef uint16_t pgn_t;
typedef struct pgRegistry_s {
    pgn_t pgn;             // vvvv iiii iiii iiii: The parameter group number, the top 4 bits 
                           // are reserved for version, and the low 12 bits for IDs.
...
#define PG_REGISTER_ATTRIBUTES  __attribute__ ((section("pg_registry_data"),  used, aligned(4)))
#define PG_RESETDATA_ATTRIBUTES __attribute__ ((section("pg_resetdata_data"), used, aligned(2)))
...
// Config must be registered with PG_REGISTER_WITH_RESET_TEMPLATE macro
...
#define PG_RESET_ARRAY_TEMPLATE(_type, _size, _name, ...)                            \
    const _type pgResetTemplate_ ## _name[_size] PG_RESETDATA_ATTRIBUTES = {   \
        __VA_ARGS__                                                     \
    }                                                                   \
...
//bool pgResetCopy(void *copy, pgn_t pgn);      // copy is not used for temporary.
